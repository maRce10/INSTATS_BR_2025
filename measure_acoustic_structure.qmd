...
-   peakf: peak frequency. Frequency with the highest energy. This feature can take a considerable amount of time to measure. Only generated if `fast = FALSE`. It provides a more accurate measurement of the peak frequency than `meanpeakf()`, but can be more easily affected by background noise
...
`sig2noise()` measures this feature. The duration of the margin in which to measure the background noise must be provided (`mar` argument):
...
### Calculates features at higher levels of organization

Vocalizations can be organized above the basic signal units like in long repertoire songs or multi-syllable calls. We can calculate average or extreme values of acoustic features of the sub-units for these higher levels of organization using the function `song_analysis()`:
...
This can also be done on features extracted from other functions:
...
Calculate song-level features selecting features with 'mean_colm':
...
Calculate song-level features for selecting features with 'mean_colm', 'max_colm' and 'min_colm' and weighted by duration:
...
download.file(url = "https://github.com/maRce10/INSTATS_BR_2025/raw/master/examples/response_calls.RDS", 
